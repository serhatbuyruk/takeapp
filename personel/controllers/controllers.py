...
class personelProfileReq(http.Controller):

    @http.route(['/ewelink-report'], type="json", auth="public", methods=["POST"], cors='*', csrf=False)
    def ewelink_report(self):
    ...
    @http.route(['/create/personel-report'], type="json", auth="public", methods=["POST"], cors='*', csrf=False)
    def create_personel_report(self):
        try:
            # Parse the incoming JSON data
            data = json.loads(request.httprequest.data)
            _logger.info('Received data for new personnel report: %s', data)

            # Access parameters directly with 'params' key
            params = data.get('params', {})

            # Validate the presence of all necessary parameters
            required_fields = ['partner_id', 'lat', 'lng', 'parent_id']
            for field in required_fields:
                if field not in params:
                    _logger.error('Missing required field: %s', field)
                    return {"code": 400, "message": f"Missing required field: {field}"}

            # Convert the parent_id to an integer if it's a string
            if isinstance(params['parent_id'], str) and params['parent_id'].isdigit():
                params['parent_id'] = int(params['parent_id'])

            # Construct the values dictionary for record creation
            val = {
            ...
            # Create the personnel profile record
            create = request.env['personel.profile'].sudo().create(val)
            _logger.info('Personnel report created successfully with ID: %s', create.id)

            return {"code": 200, "message": "Created Successfully"}

        except Exception as e:
            _logger.exception('Failed to create personnel report due to an unexpected error: %s', e)
    ...
    def nfc_profile_analyze(self,card_id):
        personel = http.request.env['personel.profile'].sudo().search([["card_id","ilike",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if personel:
                return request.render("website.nfc-profile", {'contact': personel.card_owner, 'personel': personel})
            else:
    ...
    def old_cards_analyze(self,card_id):
        personel = http.request.env['personel.profile'].sudo().search([["card_id","=",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if personel:
                return request.render("website.nfc-profile", {'contact': personel.card_owner, 'personel': personel})
    ...
    def old_text_cards_analyze(self,card_id):
        personel = http.request.env['personel.profile'].sudo().search([["name","=",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if personel:
                return request.render("website.nfc-profile", {'contact': personel.card_owner, 'personel': personel})
    ...
    def create_connect(self, **kw):
        ...
        #      'currency': kw.get('currency_id')
        #  }
        result = http.request.env['personel.profile'].sudo().search([["card_id","ilike",kw.get('card_id')]],limit=1)
        if result:
    ...
    def create_device_req_1(self):
        data = json.loads(request.httprequest.data)
        device_id = http.request.env['personel.profile'].sudo().search([["device_id","=",data['params']['device_id']]],limit=1)
        if len(device_id) > 0:
            result = ""
