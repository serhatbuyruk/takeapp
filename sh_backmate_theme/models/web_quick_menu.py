...
class Testimonial(models.Model):
    _name = 'sh.login.testimonial'
    _description = 'Login Testimonial'
    _order = 'sequence,id'
    ...
    client_image = fields.Binary("Client Image")
...
class res_users(models.Model):
    ...
    sh_enable_full_width_form = fields.Boolean('Full Width Form')
    ...
    sh_enable_gloabl_search_mode = fields.Boolean('Global Search Mode',default=True)

    sh_enable_language_selection = fields.Boolean('Language Selection',default=True)
    sh_enable_quick_menu_mode  = fields.Boolean("Quick Menu Mode",readonly="False",default=True)
    ...
    sh_hide_full_width = fields.Boolean("Hide full width",compute='_compute_theme_style')
    ...
    def _compute_theme_style(self):
        for rec in self:
            rec.sh_hide_full_width = False
            theme_settings = self.env['sh.back.theme.config.settings'].sudo().search([('id','=',1)],limit=1)
            if theme_settings and theme_settings.theme_style == 'style_8' and theme_settings.chatter_position == 'normal':
                rec.sh_hide_full_width = True

    @property
    def SELF_READABLE_FIELDS(self):
        return super().SELF_READABLE_FIELDS + ['sh_disable_auto_edit_model','sh_enable_multi_tab','sh_enable_quick_menu_mode','sh_enable_full_width_form', 'sh_enable_night_mode','sh_enable_language_selection','sh_enable_full_screen_mode','sh_enable_calculator_mode','sh_enable_gloabl_search_mode','sh_enable_todo_mode','sh_enable_zoom','sh_enable_expand_collapse','show_attachment_in_list_view','sh_enable_open_record_in_new_tab']

    @property
    def SELF_WRITEABLE_FIELDS(self):
            return super().SELF_WRITEABLE_FIELDS + ['sh_disable_auto_edit_model','sh_enable_multi_tab','sh_enable_quick_menu_mode','sh_enable_full_width_form', 'sh_enable_night_mode','sh_enable_language_selection','sh_enable_full_screen_mode','sh_enable_calculator_mode','sh_enable_gloabl_search_mode','sh_enable_todo_mode','sh_enable_zoom','sh_enable_expand_collapse','show_attachment_in_list_view','sh_enable_open_record_in_new_tab']
    ...
    def get_attachment_data(self,model,res_ids):
        ...
        if model and res_ids and self.env.user.show_attachment_in_list_view:
            ...
            return [],False
class ShPinnedApp(models.Model):
    _name = 'sh.pinned.app'
    ...
    app_id = fields.Many2one(comodel_name = "ir.ui.menu",
                              string = "Menu",
                              ondelete='cascade')
