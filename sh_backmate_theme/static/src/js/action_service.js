...
function makeActionManager(env) {
    ...
    // Softhealer Code Start
    function _computeShStackIndex(options) {
        let index = null;
        if (options.clearBreadcrumbs) {
            index = 0;
        } else if (options.stackPosition === "replaceCurrentAction") {
    ...
    async function _updateUI(controller, options = {}) {
		...
		if(options['click_tab'] == 1){
			index = _computeShStackIndex(options);
		}else{
			 index = _computeStackIndex(options);
		}

        // $("#sh_owl_items li").each((id,elem) => {
		// 	var action = $(elem).data('action')
		// 	// controller_tab_list.push(action)
        //     if(action == controller['action']['id']){
        ...
        nextStack = controllerStack.slice(0, index).concat(controllerArray);
        ...
        if(enable_multi_tab){
            ...
            var shnextStack = controllerStack.slice(0, 0).concat(controllerArray);

            controller.config.breadcrumbs = reactive(
                action.target === "new" ? [] : _getBreadcrumbs(shnextStack)
            );
    ...
    async function _executeActWindowAction(action, options) {
        ...
        var rec= _updateUI(controller, updateUIOptions);
        // Softhealer code start
        if(enable_multi_tab){


            if(localStorage.getItem("sh_add_tab") == 1 ||  $("#sh_owl_items div").length == 0 || action.xml_id =="crm.crm_lead_action_pipeline" || action.xml_id =="crm.crm_lead_action_forecast" ){
                shAddTabElement(controller, action, options)
                
            }else{
                $('#sh_owl_items div.sh_active').removeClass('sh_active');
            }          
            
        }
        else{
            $(document).find('.o_action_manager').find('.sh_main_content').remove()
    ...
    async function shAddTabElement(controller, action, options){
        // Softhealer Code start
        ...
        if(action.target != 'new'){
           ...
           var controller_tab_list = []
           $("#sh_owl_items div").each((id,elem) => {
               var action = $(elem).data('action')
               controller_tab_list.push(action)

           });

           // Check whether tab is alredy there in list
           if(jQuery.inArray(controller['action']['id'], controller_tab_list) == -1){
               if(action.name != undefined){
                   var li_xpath ='<div class="item sh_active" data-action="'+controller['action']['id']+'" data-controller="'+controller['jsId']+'">'+action.name+'<i style="margin-left:7px;" class="fa fa-times sh_remove_tab"/></div>'
               }else if(action.res_id && action.res_model){
                   let result = await rpc.query({
                       model: action.res_model,
                       method: 'search_read',
                       domain: [['id', '=', action.res_id]],
                       fields: ['name'],
                   });
                   if(result[0] && result[0].name){
                       var li_xpath ='<div class="item sh_active" data-action="'+controller['action']['id']+'" data-controller="'+controller['jsId']+'">'+result[0].name+'<i style="margin-left:7px;" class="fa fa-times sh_remove_tab"/></div>'
                   }
               }
                
               $('#sh_owl_items div.sh_active').removeClass('sh_active');
               // $("#sh_owl_items").append(li_xpath)

               $('.owl-carousel').trigger('add.owl.carousel', [li_xpath]).trigger('refresh.owl.carousel');
               ...
               $("#sh_owl_items div").each((id,elem) => {
                   ...
                   if(action == controller['action']['id']){
                       $('#sh_owl_items div.sh_active').removeClass('sh_active');
                       $(elem).addClass('sh_active')
                       $(elem).attr("data-controller",controller['jsId'])
                   }
       
               });
           }
           var $sh_nav_item_btn =  $("#sh_owl_items").find(".owl-item").find(".item");
           var $sh_remove_tab = $("#sh_owl_items").find(".owl-item").find(".sh_remove_tab")
           $sh_nav_item_btn.on('click', function(sh_e) {
              // e.preventDefault();
               sh_e.stopImmediatePropagation();
               sh_e.stopPropagation();
               // Check if click on same tab
               if(!$(this).hasClass('sh_active')){
                   $('#sh_owl_items div.sh_active').removeClass('sh_active');
                   $(this).addClass('sh_active');
                   restore(sh_e.currentTarget.dataset.controller);
               }
               
           });
           $sh_remove_tab.on('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
               // Check if closing same tab or not
            //    if($(this).parent("div").parent('.owl-item').find('.item').hasClass('sh_active')){
            //        if($(this).parent("div").parent('.owl-item').prev().length > 0){
            //            $(this).parent("div").parent('.owl-item').prev().find('.item').click()
            //        }
            //        else if($(this).parent("div").parent('.owl-item').next().length > 0) {
            //            $(this).parent("div").parent('.owl-item').next().find('.item').click()
            //        }
            //    }
            //    $(this).parent('div').parent('.owl-item').parents().find('.sh_main_content').hide()
            //    $(this).parent('div').parent('.owl-item').parents().find('.sh_main_content').addClass('sh_no_multi_tab')
               $(this).parent('div').parent('.owl-item').remove()
    ...
    async function switchView(viewType, props = {}) {
        ...
        // Softhealer Code start
         // If view change then keep last view in store
		$("#sh_owl_items div").each((id,elem) => {
			var action = $(elem).data('action')
            if(action == controller['action']['id']){
                $(elem).attr("data-controller",newController['jsId'])
            }
