...
class realestatesWizard(models.TransientModel):
    _name = "realestates.wizard"
    _description = "Real Estates Wizard"
    ...
    def default_get(self, fields):
        defaults = super(realestatesWizard, self).default_get(fields)
        ...
        if active_id:
            ...
            realestate = self.env["realestates.profile"].browse(active_id)
    ...
    def update_realestate_information(self):
        # find active modems
        active_ids = self.env.context.get("active_ids", [])
        realestate = self.env["realestates.profile"].browse(active_ids)
        # _logger.info("X" * 50 + "\n" + str(self) + "\n" + "X" * 50)
        realestate.write(
            {
