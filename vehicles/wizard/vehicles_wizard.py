...
class vehiclesWizard(models.TransientModel):
    _name = "vehicles.wizard"
    _description = "Vehicles Wizard"
    ...
    def default_get(self, fields):
        defaults = super(vehiclesWizard, self).default_get(fields)
        ...
        if active_id:
            ...
            vehicle = self.env["vehicles.profile"].browse(active_id)
            # Set values as default
            defaults['name'] = vehicle.name
            defaults['sale_price'] = vehicle.sale_price
            defaults['sale_price_currency_id'] = vehicle.sale_price_currency_id.id
            defaults['deposit_price'] = vehicle.deposit_price
            defaults['deposit_price_currency_id'] = vehicle.deposit_price_currency_id.id
            defaults['remaining_amount'] = vehicle.remaining_amount
            defaults['remaining_amount_currency_id'] = vehicle.remaining_amount_currency_id.id
            defaults['last_received_amount'] = vehicle.received_amount
            defaults['customer_payment_status'] = vehicle.customer_payment_status
        return defaults
    ...
    def update_vehicle_information(self):
        # find active modems
        active_ids = self.env.context.get("active_ids", [])
        vehicle = self.env["vehicles.profile"].browse(active_ids)
        # _logger.info("X" * 50 + "\n" + str(self) + "\n" + "X" * 50)
        vehicle.write(
            {
        ...
        if vehicle.sale_price > 0 and vehicle.remaining_amount == 0 and vehicle.received_amount > 0:
            vehicle["customer_payment_status"] = "paid"
        if vehicle.sale_price > 0 and vehicle.remaining_amount > 0 and vehicle.received_amount > 0:
            vehicle["customer_payment_status"] = "partial"
        if vehicle.sale_price > 0 and vehicle.remaining_amount > 0 and vehicle.received_amount == 0:
            vehicle["customer_payment_status"] = "not_paid"
        #vehicles.check_context()
        # close the wizard
