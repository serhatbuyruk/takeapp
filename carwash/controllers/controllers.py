...
class rentalsProfileReq(http.Controller):

    @http.route(['/create/rentals-report'], type="json", auth="public", methods=["POST"], cors='*', csrf=False)
    def create_rentals_report(self):
        ...
        create = request.env['rentals.profile'].sudo().create(val)
    ...
    def nfc_profile_analyze(self,card_id):
        rentals = http.request.env['rentals.profile'].sudo().search([["card_id","ilike",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if rentals:
                return request.render("website.nfc-profile", {'contact': rentals.card_owner, 'rentals': rentals})
            else:
    ...
    def old_cards_analyze(self,card_id):
        rentals = http.request.env['rentals.profile'].sudo().search([["card_id","=",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if rentals:
                return request.render("website.nfc-profile", {'contact': rentals.card_owner, 'rentals': rentals})
    ...
    def old_text_cards_analyze(self,card_id):
        rentals = http.request.env['rentals.profile'].sudo().search([["name","=",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if rentals:
                return request.render("website.nfc-profile", {'contact': rentals.card_owner, 'rentals': rentals})
    ...
    def create_connect(self, **kw):
        ...
        #      'currency': kw.get('currency_id')
        #  }
        result = http.request.env['rentals.profile'].sudo().search([["card_id","ilike",kw.get('card_id')]],limit=1)
        if result:
    ...
    def create_device_req_1(self):
        data = json.loads(request.httprequest.data)
        device_id = http.request.env['rentals.profile'].sudo().search([["device_id","=",data['params']['device_id']]],limit=1)
        if len(device_id) > 0:
            result = ""
