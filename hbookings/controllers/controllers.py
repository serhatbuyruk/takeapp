...
class hbookingsProfileReq(http.Controller):

    @http.route(['/create/hbookings-report'], type="json", auth="public", methods=["POST"], cors='*', csrf=False)
    def create_hbookings_report(self):
        ...
        create = request.env['hbookings.profile'].sudo().create(val)
    ...
    def nfc_profile_analyze(self,card_id):
        hbookings = http.request.env['hbookings.profile'].sudo().search([["card_id","ilike",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if hbookings:
                return request.render("website.nfc-profile", {'contact': hbookings.card_owner, 'hbookings': hbookings})
            else:
    ...
    def old_cards_analyze(self,card_id):
        hbookings = http.request.env['hbookings.profile'].sudo().search([["card_id","=",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if hbookings:
                return request.render("website.nfc-profile", {'contact': hbookings.card_owner, 'hbookings': hbookings})
    ...
    def old_text_cards_analyze(self,card_id):
        hbookings = http.request.env['hbookings.profile'].sudo().search([["name","=",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if hbookings:
                return request.render("website.nfc-profile", {'contact': hbookings.card_owner, 'hbookings': hbookings})
    ...
    def create_connect(self, **kw):
        ...
        #      'currency': kw.get('currency_id')
        #  }
        result = http.request.env['hbookings.profile'].sudo().search([["card_id","ilike",kw.get('card_id')]],limit=1)
        if result:
    ...
    def create_device_req_1(self):
        data = json.loads(request.httprequest.data)
        device_id = http.request.env['hbookings.profile'].sudo().search([["device_id","=",data['params']['device_id']]],limit=1)
        if len(device_id) > 0:
            result = ""
