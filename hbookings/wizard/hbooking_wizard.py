...
class hbookingsWizard(models.TransientModel):
    _name = "hbookings.wizard"
    _description = "Hbookings Wizard"
    ...
    def default_get(self, fields):
        defaults = super(hbookingsWizard, self).default_get(fields)
        ...
        if active_id:
            ...
            realestate = self.env["hbookings.profile"].browse(active_id)
    ...
    def update_realestate_information(self):
        # find active modems
        active_ids = self.env.context.get("active_ids", [])
        realestate = self.env["hbookings.profile"].browse(active_ids)
        # _logger.info("X" * 50 + "\n" + str(self) + "\n" + "X" * 50)
        realestate.write(
            {
