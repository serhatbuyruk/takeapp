...
class WebStoryController(http.Controller):
    
    @http.route('/webstory/data', type='json', auth='public', cors='*', methods=['GET', 'POST'])
    def get_webstory_data(self, **kwargs):
        try:
            # webstory.profile modelinden veri çekiyoruz
            records = request.env['webstory.profile'].sudo().search([("visibility", "=", True)])
            data = [{
                'id': rec.id,
                'name': rec.name,
                'webstory_image_small': rec.webstory_image_small.decode('utf-8') if rec.webstory_image_small else None,
                'webstory_image_big': rec.webstory_image_big.decode('utf-8') if rec.webstory_image_big else None,
            } for rec in records]
            return {'data': data}
