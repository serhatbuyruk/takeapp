...
class LatestProduct extends Component {
    setup(){
        ...
        this.props.selete_data_config = "LatestProduct"
        // Search
        ...
        useSubEnv({"LatestProduct":this})
    }
    async _get_latest_product(){
        let details = await this._rpc('/get_quick_record',{'mode':'get_latest_product'})
        if(details.length != 0){
...
LatestProduct.template = 'theme_alan.s_latest_product_configuration';
LatestProduct.components = { SelectData };
