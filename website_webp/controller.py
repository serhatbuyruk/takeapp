...
class BinaryWebp(Binary):
    ...
    def content_image(
        ...
        **kwargs
        ...
        if (
            request
            and request.session.get("webp_enable", False)
            or kwargs.get("webp", False)
        ):
            ...
            if request.httprequest.user_agent and browsers_capability:
                ...
                if (
                    safari
                    and float(request.httprequest.user_agent.version) < 14
                ):
                    browsers_capability = False
                ...
                if browsers_capability:
                    ...
                    config = webp.WebPConfig.new(
                        preset=webp.WebPPreset.PHOTO, quality=q
                    )
