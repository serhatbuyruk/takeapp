...
class slotsWizard(models.TransientModel):
    _name = "slots.wizard"
    _description = "Vardiya Sihirbazı"
    ...
    def default_get(self, fields):
        defaults = super(slotsWizard, self).default_get(fields)
        ...
        if active_id:
            ...
            slot = self.env["slots.profile"].browse(active_id)
            # Set values as default
            defaults['name'] = slot.name
            defaults['sale_price'] = slot.sale_price
            defaults['sale_price_currency_id'] = slot.sale_price_currency_id.id
            defaults['deposit_price'] = slot.deposit_price
            defaults['deposit_price_currency_id'] = slot.deposit_price_currency_id.id
            defaults['remaining_amount'] = slot.remaining_amount
            defaults['remaining_amount_currency_id'] = slot.remaining_amount_currency_id.id
            defaults['last_received_amount'] = slot.received_amount
            defaults['customer_payment_status'] = slot.customer_payment_status
        return defaults

    # wizard's save button
    def update_slot_information(self):
        # find active modems
        active_ids = self.env.context.get("active_ids", [])
        slot = self.env["slots.profile"].browse(active_ids)
        # _logger.info("X" * 50 + "\n" + str(self) + "\n" + "X" * 50)
        slot.write(
            {
        ...
        if slot.sale_price > 0 and slot.remaining_amount == 0 and slot.received_amount > 0:
            slot["customer_payment_status"] = "paid"
        if slot.sale_price > 0 and slot.remaining_amount > 0 and slot.received_amount > 0:
            slot["customer_payment_status"] = "partial"
        if slot.sale_price > 0 and slot.remaining_amount > 0 and slot.received_amount == 0:
            slot["customer_payment_status"] = "not_paid"
        #slots.check_context()
        # close the wizard
