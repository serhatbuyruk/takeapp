...
class slotsProfileReq(http.Controller):
    ...
    def start_courier_slot(self, latitude=None, longitude=None, accuracy=0.0):
        courier = request.env.user.partner_id
        if courier.user_role != 'kurye':
            return {
                'status': 'error',
                'message': 'Bu işlem yalnızca kurye hesapları tarafından yapılabilir.',
            }

        line = request.env[
            'skurye.profile.lines'
        ].sudo().get_current_courier_line(courier)
        slot = (
            request.env['slots.profile'].sudo().browse(line.sequence).exists()
            if line else request.env['slots.profile']
        )
        if not line or not slot or line.start_date:
            return {
                'status': 'error',
                'message': 'Şu anda başlatabileceğiniz aktif bir vardiya bulunamadı.',
            }

        try:
            return slot.start_courier_slot(
                courier.sudo(),
                latitude,
                longitude,
                accuracy,
    ...
    @http.route(['/create/slots-report'], type="json", auth="public", methods=["POST"], cors='*', csrf=False)
    def create_slots_report(self):
        ...
        create = request.env['slots.profile'].sudo().create(val)
    ...
    def nfc_profile_analyze(self,card_id):
        slots = http.request.env['slots.profile'].sudo().search([["card_id","ilike",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if slots:
                return request.render("website.nfc-profile", {'contact': slots.card_owner, 'slots': slots})
            else:
    ...
    def old_cards_analyze(self,card_id):
        slots = http.request.env['slots.profile'].sudo().search([["card_id","=",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if slots:
                return request.render("website.nfc-profile", {'contact': slots.card_owner, 'slots': slots})
    ...
    def old_text_cards_analyze(self,card_id):
        slots = http.request.env['slots.profile'].sudo().search([["name","=",card_id]],limit=1)
        qr = http.request.env['qidgenerator.qidgenerator'].sudo().search([["card_id","=",card_id]],limit=1)
        if qr:  
            if slots:
                return request.render("website.nfc-profile", {'contact': slots.card_owner, 'slots': slots})
    ...
    def create_connect(self, **kw):
        ...
        #      'currency': kw.get('currency_id')
        #  }
        result = http.request.env['slots.profile'].sudo().search([["card_id","ilike",kw.get('card_id')]],limit=1)
        if result:
    ...
    def create_device_req_1(self):
        data = json.loads(request.httprequest.data)
        device_id = http.request.env['slots.profile'].sudo().search([["device_id","=",data['params']['device_id']]],limit=1)
        if len(device_id) > 0:
            result = ""
