...
class cordersProfileReq(http.Controller):
    ...
    def make_available(self, kurye_id):
        ...
        reason = ""
        if str(courier.id) != str(kurye_id):
            reason = "Başka bir kurye adına vardiya başlatamazsınız."
        elif (
            not courier.son_konum_zamani
            or courier.son_konum_zamani < fields.Datetime.now() - timedelta(minutes=2)
        ):
            reason = (
                "Güncel konum alınamadı. Ana sayfadaki Vardiyayı Başlat "
                "butonunu kullanarak konum izni verin."
            )
        else:
            now = fields.Datetime.now()
            slot = request.env['slots.profile'].sudo().search([
                ('active_status', '=', True),
            ...
            if slot:
                result = slot
                try:
                    start_result = slot.start_courier_slot(
                        courier.sudo(),
                        courier.lat,
                        courier.lng,
                        courier.konum_dogrulugu,
                    )
                    mesafe = start_result['distance']
    ...
    def make_busy(self, kurye_id):
        ...
        if len(kurye) > 0:
            current_line = request.env[
                'skurye.profile.lines'
            ].sudo().get_current_courier_line(kurye)
            current_slot = (
                request.env['slots.profile'].sudo()
                .browse(current_line.sequence).exists()
                if current_line
                else request.env['slots.profile']
            )
            if (
                current_line
                and current_line.start_date
                and current_slot
                and current_slot.slot_tipi == 'sabit'
            ):
                _logger.warning(
                    "Aktif sabit slot sırasında /busy çağrısı engellendi: "
                    "kurye=%s slot=%s",
                    kurye.id,
                    current_slot.id,
                )
        ...
        if len(siparis) > 0:
            if int(current_user_id) == int(user_id):
                ...
                if siparis.pos_entegrasyon_firmasi == "pagate":
                    ...
                    kurye_response = requests.put(url, headers=headers, json=payload, timeout=HTTP_TIMEOUT)
                    create_ir_logging = (
                            request.env['ir.logging']
                            .sudo()
                            .create(
                ...
                if siparis.pos_entegrasyon_firmasi == "adisyo" and siparis.pos_entegrasyon_id != False:
                    ...
                    if adisyo_kurye_id == False:
                        ...
                        kurye_response = requests.get(url, headers=headers, json=payload, timeout=HTTP_TIMEOUT)
                        create_ir_logging = (
                                request.env['ir.logging']
                                .sudo()
                                .create(
                    ...
                    if adisyo_kurye_id > 0:
                        ...
                        response = requests.post(url, verify=True, headers=headers, json=payload, timeout=HTTP_TIMEOUT)
    ...
    def get_kurye_siparisi_teslim_etti(self, corder_id, current_user_id):
        user_id = request.session.uid
        siparis = http.request.env['corders.profile'].sudo().search(
            [('id', '=', corder_id)], limit=1
        )
        if siparis and int(current_user_id) == int(user_id):
            # Teslimatın Odoo içindeki durumu dış POS servisinin erişilebilirliğine
            # bağlı olmamalıdır. Bu yazma işlemi ilgili automated action üzerinden
            # siparis_durumu alanını da "teslim_edildi" yapar.
            ...
            try:
                if siparis.pos_entegrasyon_firmasi == "yeppos":
                    ...
                    if siparis.magaza.yeppos_url:
                        url = siparis.magaza.yeppos_url
                        api_key = siparis.magaza.yeppos_api_key
                    else:
                        url = "%s/v1/couries/%s" % (
                            company.x_yeppos_url.rstrip('/'),
                            siparis.yeppos_order_id,
                        )
                        api_key = company.x_yeppos_api_key
                    ...
                    _logger.info(
                        "Yeppos delivery callback for order %s returned HTTP %s",
                        siparis.id,
                        response.status_code,
                    )

                elif siparis.pos_entegrasyon_firmasi == "sepettakip":
                    company = request.env.company.sudo()
                    response = requests.patch(
                        "%s/courier-company/order" % company.x_sepettakip_url.rstrip('/'),
                        verify=True,
                        headers={
                            'Courier-Company': company.x_sepettakip_courier_company,
                            'Api-Key': company.x_sepettakip_api_key,
                    ...
                    _logger.info(
                        "Sepettakip delivery callback for order %s returned HTTP %s",
                        siparis.id,
                        response.status_code,
                    )

                elif siparis.pos_entegrasyon_firmasi == "pagate":
                    response = requests.put(
                        "https://possiweb.com/api/v2/provider/update-order",
                        headers={
                            'Content-Type': 'application/json',
                            'Authorization': siparis.magaza.token,
                        },
                        json={
                            "created_by": "kuryetec",
                            "order_id": str(siparis.remoteId),
                            "status_code": "delivered",
                        },
                        timeout=HTTP_TIMEOUT,
                    )
                    _logger.info(
                        "Pagate delivery callback for order %s returned HTTP %s",
                        siparis.id,
                        response.status_code,
                    )

                elif (
                    siparis.pos_entegrasyon_firmasi == "adisyo"
                    and siparis.pos_entegrasyon_id
                    and siparis.adisyo_payment_method_id
                ):
                    response = requests.post(
                        "https://ext.adisyo.com/api/External/v2/Deliver",
                        verify=True,
                        headers={
                            'x-api-key': siparis.magaza.adisyo_x_api_key,
                            'x-api-secret': siparis.magaza.adisyo_x_api_secret,
                        ...
                        json={
                            "OrderId": int(siparis.pos_entegrasyon_id),
                            "PaymentType": int(siparis.adisyo_payment_method_id),
                        },
                        timeout=HTTP_TIMEOUT,
                    )
                    _logger.info(
                        "Adisyo delivery callback for order %s returned HTTP %s",
                        siparis.id,
                        response.status_code,
                    )
            except (requests.RequestException, TypeError, ValueError, AttributeError):
                # POS bildirimi başarısız olsa bile kurye ekranında teslim işlemi
                # 500 vermemeli ve Odoo teslimat kaydını geri almamalıdır.
                _logger.exception(
                    "%s delivery callback failed for order %s; local delivery is preserved",
                    siparis.pos_entegrasyon_firmasi or "local",
                    siparis.id,
                )
        return request.render("website.kurye-anasayfa")
