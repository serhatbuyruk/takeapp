...
class citizenshipsWizard(models.TransientModel):
    _name = "citizenships.wizard"
    _description = "Citizenships Wizard"
    ...
    def default_get(self, fields):
        defaults = super(citizenshipsWizard, self).default_get(fields)
        ...
        if active_id:
            ...
            realestate = self.env["citizenships.profile"].browse(active_id)
    ...
    def update_realestate_information(self):
        # find active modems
        active_ids = self.env.context.get("active_ids", [])
        realestate = self.env["citizenships.profile"].browse(active_ids)
        # _logger.info("X" * 50 + "\n" + str(self) + "\n" + "X" * 50)
        realestate.write(
            {
