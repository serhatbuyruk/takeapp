...
class ProcessBackupDetail(models.Model):
    ...
    def download_db_file(self):
        ...
        """
        try:
            if self.status == "Success" and os.path.exists(self.url):
    ...
    def unlink_confirmation(self):
        for rec in self:
            if rec.status=="Success":
                msg = """ <span><strong>Warning:</strong> After Deleting this record you will no longer be able to download the backup file associated with this record. However, after deletion the backup will still remain on server.
